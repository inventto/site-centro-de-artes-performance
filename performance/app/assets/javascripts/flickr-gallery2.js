...
(function(jQuery) {
	...
	jQuery.fn.flickrGallery = function(args) {
			...
			init : function () {
               ...
               if (!(settings.flickrUser === null)) {
                    $.getJSON("http://api.flickr.com/services/rest/?format=json&method=flickr.photosets.getList&user_id=" + settings.flickrUser + "&api_key=" + settings.flickrKey + "&jsoncallback=?", function(flickrData){
                        ...
                        $("#container").append('<div id="flickr_sets" class="box"></div>');
		...
		removerUltimoCaracter = function(string){
			return string.substring(0, string.length -1);
		};
		setSlideImage = function(imagem, max_imagens){
			
			if($('.slide-flex-fotos[data-thumb="'+ imagem +'"]').empty()) {
				    $('.slides').append("<li class='slide-flex-fotos' data-thumb="+ imagem+">"+"</li>");
				    console.log("SETTING IMG", imagem);
			};
		 	if ($('img[src="'+ imagem+'"]').empty()) {
		 		  	console.log("Entrou com", imagem);
		 		    $(".flexslider .slides li").append('<img src=' + imagem + '/>');
		 	};
		 	imagens = $(".slide-flex-fotos img");
		 	if (imagens.length > max_imagens){
		 		var count = 0;
		 		/*for (var i = 0; i < imagens.length ; i++) {
		 			var img = imagens[i];
		 			//if (count != 3){
		 			  if (removerUltimoCaracter($(img).attr("src")) == imagem) {
						img.remove();
		 				//count ++;
				    ...
				    /*$('#slider .slides').append("<li></li>");
				    $('#slider .slides li').append("<img src="+ imagem +">");
				    console.log("SETTING IMG", imagem);
			
		 	
		 		  	console.log("Entrou com", imagem);
		 		    $('#carousel .slides').append("<li></li>");
				    $('#carousel .slides li').append("<img src="+ imagem +">");*/
        ...
        loadFlickrSet = function() {
				...
				$.getJSON("http://api.flickr.com/services/rest/?format=json&method=flickr.photosets.getPhotos&photoset_id=" + settings.flickrSet + "&api_key=" + settings.flickrKey + "&jsoncallback=?", function(flickrData){
					...
					var thumbHTML = '';
					$('#flickr_sets').after("<div class='flexslider'></div>");
				    $('.flexslider').append("<ul class='slides'></ul>");

					//$('#flickr_sets').after("<div id='slider' class='flexslider'></div>");
				    //$('.flexslider').append("<ul class='slides'></ul>");
					...
					for (i=0; i<length; i++) {
						...
						//setSlideImage(photoURL);
						//$('.slides').append("<li class='slide-flex-fotos' data-thumb="+settings.imgArray[i]+">"+"</li>");
						//$(".slide-flex-fotos").append('<img src=' + settings.imgArray[i] + '/>');
						//console.log("PHOTO URL", photoURL.length);
					}
		...
		flickrhelpers = {
			navImg : function (index) {
				...
				// Display the image
				element.html('');
				console.log("SETTINGS LENGHT", settings.imgArray.length);
				for (var i =0; i < settings.imgArray.length; i++){
				  setSlideImage(settings.imgArray[i],settings.imgArray.length);
			    }
